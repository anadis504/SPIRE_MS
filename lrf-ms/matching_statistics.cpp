...
void print_help(char **argv) {
  std::cout
      ...
      << "OR: "
      << "Usage: " << argv[0]
      << " -z input_file (which is a concatenation of strings separated "
         "by \'$\' and the first string is used as the reference) [options]"
      << std::endl;
  std::cout
      << "  Options: " << std::endl
...
void parseArgs(int argc, char **argv, Args &arg) {
  ...
  std::string sarg;
  while ((c = getopt(argc, argv, "r:c:z:o:b:t:h")) != -1) {
    switch (c) {
      case 'r':
      ...
      case 'c':
        ...
        break;
      case 'z':
        arg.concatenation.assign(optarg);
        break;
      case 'o':
...
void computeMatchingStatistics(std::string collFileName, uint64_t bufferText,
  ...
  if (matchingStatistics.size() != 0) {
    streamOutfile.write(
        reinterpret_cast<char *>(matchingStatistics.data()),
        matchingStatistics.size() * sizeof(std::pair<uint32_t, uint32_t>));
    matchingStatistics.clear();
  }
  streamOutfile.close();
...
#pragma omp parallel for reduction(+ : nCMSInBuffer)
    ...
    if (CMStotalBuffer.size() >= capBufferWrite) {
      ...
      for (uint64_t i = 1; i < CMStotalBuffer.size(); i++) {
        ...
        streamOutfile.write(
            reinterpret_cast<char *>(msBuffer.data()),
            msBuffer.size() * sizeof(std::pair<uint32_t, uint32_t>));
        msBuffer.clear();
      }
...
std::vector<char> read_file(const char *filename) {
  ...
  const std::size_t len = (end - begin) / sizeof(char);
  ifs.seekg(0);
  std::vector<char> v(len, 0);
  for (std::size_t i = 0; i < len; ++i) {
    ifs.read(reinterpret_cast<char *>(v.data() + i), sizeof(char));
  }
  ifs.close();
  return v;
}
...
int main(int argc, char **argv) {
  ...
  std::string concFileName = arg.concatenation;
  if (!concFileName.empty()) {
    bool ref = true;
    referenceFileName = concFileName + "_ref";
    collFileName = concFileName + "_coll";

    std::ofstream ref_ofs(referenceFileName);
    std::ofstream coll_ofs(collFileName);
    ref_ofs << ">seqref\n";
    coll_ofs << ">\n";
    auto input_file = read_file(concFileName.c_str());
    for (size_t i = 0; i < input_file.size(); ++i) {
      if (input_file[i] == '$') {
        ...
        if (ref) {
          ref_ofs << input_file[i];
        } else {
          coll_ofs << input_file[i];
        }
      }
    }
    coll_ofs << "\n";
